...
| `/dashboard` | 店铺信息、工作台统计和快捷入口 |
| `/products` | 商品列表、服务端搜索/筛选/分页、批量提交审核、批量下架、SKU 库存调整 |
| `/products/create` | 发布商品，主图 URL 校验、预览和默认图 |
| `/products/:spuId/edit` | 编辑商品，展示已有主图预览 |
| `/orders` | 订单列表、搜索、状态筛选、发货 |
| `/after-sales` | 售后列表、搜索、状态筛选、同意/拒绝售后、退货验收 |
...
- 售后处理：商家可处理 `APPLIED` 售后，同意后进入退款或退货流程，拒绝时必须填写原因。
...
3. 进入 `/dashboard`，查看店铺信息、统计卡片和快捷入口。
4. 进入 `/products`，测试服务端搜索、状态/类目筛选、分页、批量提交审核、批量下架、编辑、下架、提交审核按钮显示规则和 SKU 库存调整。
